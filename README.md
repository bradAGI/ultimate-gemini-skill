...
Flags are kebab-case (e.g. `--enable-google-search`, `--reference-image-paths`). Run with `--help` for the full surface.
...
Always add `.env`, `*.key`, and agent config files to `.gitignore`. Rotate immediately on exposure.
...
├── scripts/
│   └── gemini_image.py      # standalone CLI
├── references/
...
Derived from [`anand-92/ultimate-image-gen-mcp`](https://github.com/anand-92/ultimate-image-gen-mcp) by [@anand-92](https://github.com/anand-92) — the original upstream project whose tool design and 26 prompt templates this skill adapts. Please star the upstream.
